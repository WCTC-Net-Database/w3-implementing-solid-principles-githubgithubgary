...
using W3_Linq_SRP.Interfaces;
using IOutput = W3_Linq_SRP.Interfaces.IOutput;
using System.ComponentModel.Design;

namespace W3_Linq_SRP.Models;
...
public class CharacterManager
{
    ...
    private List<PlayerCharacter> campaignList = new List<PlayerCharacter>();
        ...
        if (max_count > 0)
        {
            ...
            if (endInput != true)
            {
                ...
                if (AddCharacter)
                {
                    ...
                    campaignList.Add(new PlayerCharacter(character.Name, character.ClassName, character.Level));
                    campaignList[campaignList.Count() - 1].Id = campaignList.Count();
                    campaignList[campaignList.Count() - 1].Equipment = character.Equipment;
                    _updates = true;
                }
                else
                {
    ...
    {
        //###################################################################
        //https://stackoverflow.com/questions/45239889/linq-case-insensitive
        //var foundCharacter = campaignList.Where(c => c.Name == "Bob").ToList();
        //###################################################################
        ...
        while (true)
        {
            ...
            else
            {
                ...
                {
                    foundCharacters = campaignList.Where(c => c.Name.IndexOf(input, StringComparison.InvariantCultureIgnoreCase) >= 0).ToList();
                    //CharacterReader cr = new CharacterReader();
                    //foundCharacters = cr.CharacterSearch(input, campaignList);
    ...
    public void SaveCharacters()
    {
        ...
        else
        {
            ...
            if (_updates == true)
            {
                ...
                cw.characterlist = campaignList;
            }
            else
            {
                _output.WriteLine("Your campaign is enshrined in the scrolls of history.");
            }
            Thread.Sleep(4000);
        }
    }
        ...
        while (endInput != true)
        {
            ...
            if (campaignList.Count() > 0)
            {
                ...
                else
                {
                    ...
                    if (success == true)
                    {
                        if (string.Compare((choice - 1).ToString(), (campaignList.Count() - 1).ToString()) > 0)
                        {
                        ...
                        else
                        {
                            switch (input)
                            {
                                ...
                                default:
                                    ...
                                    {
                                        LevelUpCharacter(campaignList[idx - 1]);
                                        endInput = true;
                                        break;
                                    }
                                    else
